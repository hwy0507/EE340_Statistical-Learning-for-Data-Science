...
SAVE_DIR  = '/home/nkd/ouyangzl/Project/Bonus_2/feature/distributions'
os.makedirs(SAVE_DIR, exist_ok=True)

# 加载原始 MNIST（不作 transform）
mnist_full = datasets.MNIST(root=DATA_ROOT, train=True, download=True, transform=None)
mnist_test = datasets.MNIST(root=DATA_ROOT, train=False, download=True, transform=transforms.ToTensor())
# 采样方便可视化，2000 个训练，2000 个测试
train_subset = Subset(mnist_full, list(range(2000)))
...
# 主循环：每种增强画一张图，含 4 子图：Train-TSNE, Train-UMAP, Test-TSNE, Test-UMAP
for name, aug_fn in methods.items():
    ...
    out_path = os.path.join(SAVE_DIR, f"{name}_distribution.png")
