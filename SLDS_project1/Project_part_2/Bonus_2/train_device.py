...
for name, aug_fn in methods.items():
    ...
    # 组合 transform，对 original 和 all_transforms 不重复 ToTensor
    if name in ['original', 'all_transforms']:
...
# 左图：Loss
for name in methods:
    ax1.plot(epochs, all_metrics[name]['train_loss'], label=f"{name} Train", linestyle='-')
    ax1.plot(epochs, all_metrics[name]['test_loss'],  label=f"{name} Test" , linestyle='--')
...
# 右图：Accuracy
for name in methods:
    ax2.plot(epochs, all_metrics[name]['train_acc'], label=f"{name} Train", linestyle='-')
    ax2.plot(epochs, all_metrics[name]['test_acc'],  label=f"{name} Test" , linestyle='--')
